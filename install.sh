...
# Best-effort detection of the host's IANA timezone (systemd, then /etc/timezone,
# then the /etc/localtime symlink). Empty if it can't be determined.
detect_tz() {
  local tz=""
  if command -v timedatectl &>/dev/null; then
    tz=$(timedatectl show -p Timezone --value 2>/dev/null || true)
  fi
  [[ -z "$tz" && -f /etc/timezone ]] && tz=$(cat /etc/timezone 2>/dev/null || true)
...
# Timezone — sets when each day rolls over for recurring tasks, the daily board,
# and the morning digest. The container clock is UTC, so this must be your real
# home zone or "today" turns over at the wrong hour. Auto-detected from the host.
echo ""
dim "Your home timezone (IANA name, e.g. America/Toronto). It decides when the day"
dim "rolls over for recurring tasks and the morning digest. Changeable later in"
dim "Settings → Notifications → Timezone. Travelling devices adjust automatically."
DETECTED_TZ="$(detect_tz)"
ask_default "Timezone" SERVER_TZ "${DETECTED_TZ:-UTC}"
if [[ ! -e "/usr/share/zoneinfo/${SERVER_TZ}" ]]; then
  warn "'${SERVER_TZ}' isn't a recognised zoneinfo name — Sempa will fall back to UTC if it's invalid."
fi
